...
from dependencies.database import get_async_db, get_db
...
async def create_organisation(
    ...
    new_organisation = await service.create_organisation(owner_id, organisation_data)
...
async def get_organisation(
    ...
    owner_id = current_user["id"]
    try:
        organisation = await service.get_organisation(owner_id, organisation_id)
...
async def update_organisation(
    ...
    organisation_data = Organisation.model_validate(organisation)
    try:
        updated_organisation = await service.update_organisation(
...
async def delete_organisation(
    ...
    try:
        if not await service.delete_organisation(owner_id, organisation_id):
